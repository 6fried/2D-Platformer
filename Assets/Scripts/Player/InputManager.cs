...
public class InputManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Description:
    /// Collects leftClickInputs
    /// Input:
    /// CallbackContext callbckContext
    /// Return:
    /// void (no return)
    ///</summary>
    /// <param name="callbackContext">he context of the mouse input</param>
    public void GetMouseClickInput(InputAction.CallbackContext callbackContext)
    {
        if (callbackContext.canceled && brickSpawn != null)
        {
            Vector3 mousePosition = Mouse.current.position.ReadValue();
            Vector3 spawnPosition = sceneCamera.ScreenToWorldPoint(mousePosition);
            brickSpawn.SpawnBrick(spawnPosition);
        }
    }
